...
#To use relative path expression
...
def load_config():
    ...
    parser.add_argument('--config', type=str,default = ROOT/'configs/mstar/resnext.yaml' )
    ...
    #config.train.output_dir = str(ROOT/"experiments/mstar/resnext/exp3")
    if config.train.distributed == True:
        if 'RANK' in os.environ:
            config.merge_from_list(['train.dist.rank', int(os.environ['RANK'])])
        if 'WORLD_SIZE' in os.environ:
            config.merge_from_list(['train.dist.world_size', int(os.environ['WORLD_SIZE'])])
        if 'LOCAL_RANK' in os.environ:
            config.merge_from_list(['train.dist.local_rank', int(os.environ['LOCAL_RANK'])])
            local_rank = int(os.environ['LOCAL_RANK'])
            torch.cuda.set_device(local_rank)
            config.device = f'cuda:{local_rank}'
        if 'MASTER_ADDR' in os.environ:
            config.merge_from_list(['train.dist.master_addr', os.environ['MASTER_ADDR']])
        if 'MASTER_PORT' in os.environ:
            config.merge_from_list(['train.dist.master_port', os.environ['MASTER_PORT']])
    ...
    for step, (data, targets) in enumerate(train_loader):
        ...
        if config.train.distributed:
            ...
            acc5_all_reduce = dist.all_reduce(acc5,
                                              op=dist.ReduceOp.SUM,
                                              async_op=True)
            loss_all_reduce.wait() #type: ignore
            acc1_all_reduce.wait()#type: ignore
            acc5_all_reduce.wait()#type: ignore
    ...
    if get_rank() == 0:
        if not config.train.resume and output_dir.exists():
            user_input = input(
                ...
                'Enter 1 to delete it and start training, or any other key to exit: '
            )
            if user_input.strip() == '1':
                ...
                print(f'Deleted existing directory and created a new one: {output_dir.as_posix()}')
            else:
                raise RuntimeError('Training aborted by user.')
        output_dir.mkdir(exist_ok=True, parents=True)
